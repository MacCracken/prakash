...
| `ray` | ray_core, ray_fresnel, ray_trace, ray_simulate, ray_system, ray_dispersion, ray_fiber, ray_grin | 1155 | `Medium`, `ComplexMedium`, `*Coefficients`, `TraceRay`, `OpticalSurface`, `PolarizedTraceHit`, `ParaxialRay`, `Prescription`, `GrinProfile` | Geometric optics: Snell, Fresnel (real + complex), dispersion (Sellmeier/Cauchy/Herzberger/Schott/Conrady, analytic derivatives and group index), chromatic aberration, fiber optics and fiber chromatic dispersion (material, waveguide, total; exact LP01), sequential/recursive tracing with polarization, ray fans, spot diagrams, OPD; gradient-index tracing (SELFOC, sech, polynomial, Luneburg, fish-eye) by RK-Nyström with optical path |
...
**Total**: 28 science modules + error, **9161 test assertions across 35 suites**,
159 benchmarks. (`tests/hardening.tcyr` is the cross-module regression suite for the
