...
| **V2.3.0** | **Toolchain + hisab 3.x** | **cyrius 6.6.2 → 6.6.4, hisab 2.11.2 → 3.1.1, sakshi 2.4.11 → 2.5.2, ganita → 1.2.5 via a full `lib/` re-sync to the 6.6.4 snapshot. ⚠ **hisab 3.0.0 is breaking and the compiler is silent about it**: 48 functions moved from integer error codes to the stdlib `Result<T,E>`, `num_fft`/`num_ifft` among them, and prakash's four call sites all *discarded* the return — which still compiles. The bump built, linked and passed all 6420 existing assertions with the error channel dropped on the floor; it was found by reading the upstream changelog, not by the gate. `_pat_fft2d` now propagates `PK_ERR_INVALID_PARAMETER` instead of handing a **half-transformed grid** to the intensity squarer. ⚠ The repair was initially **mutation-invisible** — the public entry points pad every dimension through `_pat_next_pow2`, so nothing could reach the Err path and deleting the new check left the suite green at 572/572; the tests now call `_pat_fft2d` directly and both mutants die on the assertion that encodes them. Grep confirmed **zero** hisab calls in argument position, the shape whose `Result` degrades silently to its tag. ganita 1.2.5's zero-base `f64_pow` re-measured directly (the shim masks it from `hardening.tcyr`) and unchanged bit-for-bit. No performance claim: median −0.99% over 36 rows with mixed signs, movers in the sub-100 ns 40%-spread band. 6420 → 6438 assertions, 29 suites, coverage 412/412. See the [2.3.0] CHANGELOG entry.** |
| V2.2.9 | Toolchain bump | cyrius 6.5.33 → 6.6.2. No source change. ⚠ Its CHANGELOG entry was appended below the `0.1.0` notes at the foot of the file instead of at its head, so the project's most recent release was invisible from the top of the changelog for four commits; moved into order in 2.3.0. |
| **V2.2.8** | **P(-1) review: four of the last three releases' fixes were wrong** | **Eight reviewers over the whole tree plus batched adversarial verification: 18 findings confirmed, 0 refuted. ⚠ **Four defects were INTRODUCED by 2.2.6/2.2.7 — the hardening releases — and in two cases the tests written at the time pinned the wrong value.** The IBL Smith k was squared twice (k = alpha^2/2 not alpha/2), so the split-sum returned **A+B = 1.304** — energy gain — and the invariant 2.2.7 used to 'settle' the k cannot see it, since both remaps give k=0 at roughness 0 and k=0.5 at roughness 1. The Seidel q^2 term was still wrong after 2.2.6 and 2.2.6's check was **circular** (argmin compared against a formula from the same bracket); best-form singlet is 0.7143, shipped 1.4286. Both coma coefficients were wrong: coma zero at 0.55 vs the true aplanatic 0.80. CRI's reference came from McCamy so a 2000K blackbody scored Ra=99.10 instead of 100; now golden-section refined, every blackbody exactly 100.0000 and Illuminant A 100.000000. The scotopic tail 2.2.7 chose to leave was wrong in all 26 entries — the 650/655 ratio collapsed from 0.674 to 0.369 and the curve hit exact zero above 710nm. TIR was treated as a MISS so rays passed through prisms unreflected; TIR inside a layer gave NaN for s-pol and deleted the layer for p-pol — frustrated-TIR tunnelling now works (R_s 0.0055 at 10nm to 0.99999 at 1000nm). Plus the Spd family's null deref (reachable from untrusted JSON), a 32-byte OOB write in point_source_set reported as PK_ERR_NONE, and four more null/validation guards. CRI 202 -> 44.5us and 23.6KB -> 2.2KB per call (bit-identical), then 44.5 -> 290us for CIE 13.3 conformance. 5775 -> 6391 assertions; coverage 412/412 (100%) for the first time. See the [2.2.8] CHANGELOG entry.** |
| **V2.2.7** | **All seven deferred findings closed** | **Closes ALL seven of 2.2.6's deferred findings plus a sixth defect found while investigating. ⚠ **Three more inherited defects** (running total nine) — TMM signs, split-sum fit and split-sum integrator are identical in the deleted Rust archive; so is the wrong F11 table. Transfer-matrix M12/M21 cross terms had the wrong sign, making multilayer coatings up to **4.75x** wrong — invisible because both signs flip together and the reflectance squares them. `pbr_split_sum_scale_bias`'s bias was **identically zero at all 110 grid points**; `pbr_integrate_brdf_lut` used the direct-lighting Smith k where IBL needs alpha^2/2 (A+B was 0.2215 at roughness 0, must be 1). `_vlambda_scotopic` was not CIE 1951 V'(lambda) — 30 of 81 entries corrected. ⭐ **`illuminant_f11` was not F11** (lines at 440/565/615 vs 436/546/611) — 75 of 81 entries replaced from CIE 15:2004; F11 chromaticity 0.46141 → 0.38054 against a published 0.38052, and CCT lands at 4007 K (nominal 4000) as an independent check. ⭐ **`color_rendering_index` is now real CIE 13.3 Ra** — 8 Test Colour Samples, CIE 1960 UCS, von Kries adaptation, W*U*V*, Planckian/D-series references. F2 Ra 98.29 → **64.20** (published 64), F11 → 82.85 (83), D65 → 100.00. New `cri_special` exposes per-sample R_i; F2's R8=33.3 reproduces the documented deep-red failure. 891 new constants, parsed PROGRAMMATICALLY from the CIE datasets after three hand transcriptions disagreed with each other. ⚠ The last two were nearly shelved as 'needs the CIE datasets' without checking the Rust archive or the public data — both were reachable. `CLAUDE.md` converted to Cyrius. 5615 → 5775 assertions. See the [2.2.7] CHANGELOG entry.** |
